module Spree
  ...
  class CheckoutController < Spree::StoreController
    ...
    def ensure_valid_state
      # If we have no state parameter, we just transitioned from cart. Everything is fine.
      return if params[:state].blank?

      # If the current orders state has been set to something that not the current, but a future checkout state
    ...
    def set_state_if_present
      ...
      elsif !@order.has_checkout_step(params[:state]) && !skip_state_validation
        params[:state] = nil
      end
    end
